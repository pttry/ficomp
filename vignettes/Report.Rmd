...
Economic outlook antaa alkupäässä ja lopussa (?) hieman erilaisia arvoja. Tätyy tarkistaa miksi.
...
bind_rows("Eurostat ja OECD QNA" = q_dat,
          ...
          .id = "source") %>% 
  filter(geo == "FI", time >= start_time, time <= max_time_q) %>% 
...
q_dat %>% 
  ...
  translate(vars, c(change = "Muutos", values = "Taso")) %>% 
...
q_dat %>% 
  ...
  mutate(inv_nulc_aper_rel_imf = rebase(1/nulc_aper_rel_imf, time, base_year),
         XPERF = 100* XPERF) %>% 
  select(-nulc_aper_rel_imf) %>% 
  gather(vars, values, -geo, -time) %>% 
  # mutate(h_geo = fct_other(geo, keep = h_geos,other_level = "muut"),
  #        geo = fct_relevel(geo, h_geos, after = Inf)) %>% 
