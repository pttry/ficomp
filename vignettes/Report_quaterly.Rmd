...
data_quartely_est %>% 
  ...
  # group_by(time, nace0) %>% 
  # mutate(nulc_va_eur_long_rel_ecfin14_extra = weight_index2(nulc_va_eur_long, geo, time, 
  #                                                           geos = setdiff(eurostat_geos, "NL"), weight_df = weights_ecfin37)) %>% 
  # ungroup() %>% 
  select(geo, time, nulc_aper_eur_long_rel_ecfin20, REER_IC37_ULCT) %>% 
  filter(geo == "FI",
         time >= start_time) %>% 
  gather(ind, values, -geo, -time) %>% 
