...
data_tuku_ilc <- dat_ilc %>%
  ...
  summarise(
    ...
    manu_ex26 = manu - values[nace == "26-27"]) %>%
  ungroup() %>%
  gather(nace0, values, manu, manu_ex26) %>%
  ...
  group_by(nace0) %>%
  weight_at(geo, time, at = c("nulc_va"), weight_df = weights_ecfin42) %>%
  ungroup()




usethis::use_data(data_tuku_ilc, overwrite = TRUE)


library(ggplot2)

data_tuku_ilc %>%
  ggplot(aes(time, nulc_va_rel, colour = geo)) +
  facet_wrap(~nace0) +
  geom_line()
...
data_tuku_ilc %>%
  ...
  filter(vars %in% c("nulc_va_rel_ecfin42")) %>%
  ggplot(aes(time, values, colour = nace0)) +
  # facet_grid(vars~nace0, scales = "free") +
  geom_line()
